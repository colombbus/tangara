...
abstract class AbstractPageDumper {
	...
	protected void fixPageLinkUrls() {
		...
		for (int idx = 0; idx < aElements.getLength(); idx++) {
			...
			else if (isExternalAnchorElement(aElement))
				fixExternalAnchorElement(aElement);
		}
	}
	...
	private boolean isExternalAnchorElement(Element anchor) {
		if( noHref(anchor))
			return false;
		...
		System.err.println("--- anchor "+hrefUrl);
		return false;
	}

	private static boolean noHref(Element anchor ) {
		String hrefUrl = anchor.getAttribute(HREF_A);
		return hrefUrl == null || hrefUrl.isEmpty();
	}

	private static boolean localAnchor(Element anchor) {
		String hrefUrl = anchor.getAttribute(HREF_A);
		return hrefUrl.startsWith("#");
	}

	private void fixExternalAnchorElement(Element anchor) {
